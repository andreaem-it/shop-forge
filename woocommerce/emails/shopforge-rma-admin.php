...
<p><?php printf( esc_html__( 'Il cliente ha aggiornato lo stato della richiesta RMA #%d per l\'ordine #%s.', 'shopforge' ), (int) ( $rma_data['request_id'] ?? 0 ), esc_html( $order->get_order_number() ) ); ?></p>
<?php else : ?>
<p><?php printf( esc_html__( 'Nuova richiesta di %1$s ricevuta per l\'ordine #%2$s.', 'shopforge' ), esc_html( $tipo_label ), esc_html( $order->get_order_number() ) ); ?></p>
<?php endif; ?>
...
<table cellspacing="0" cellpadding="6" style="width:100%;border:1px solid #e0e0e0;border-collapse:collapse;margin-bottom:20px;">
	...
	<tr>
		<th style="text-align:left;border:1px solid #e0e0e0;padding:8px 12px;background:#f8f8f8;"><?php esc_html_e( 'Tipo', 'shopforge' ); ?></th>
	...
	<tr>
		<th style="text-align:left;border:1px solid #e0e0e0;padding:8px 12px;background:#f8f8f8;"><?php esc_html_e( 'Descrizione problema', 'shopforge' ); ?></th>
...
<p>
	<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=shopforge_rma_request' ) ); ?>"
	   style="display:inline-block;background:#96588a;color:#fff;padding:10px 20px;text-decoration:none;border-radius:4px;font-weight:bold;">
		<?php esc_html_e( 'Gestisci richiesta →', 'shopforge' ); ?>
	</a>
</p>
