...
<p><?php printf( esc_html__( 'New support request received for order #%s.', 'shopforge' ), esc_html( $order->get_order_number() ) ); ?></p>
...
<table cellspacing="0" cellpadding="6" style="width:100%;border:1px solid #e0e0e0;border-collapse:collapse;margin-bottom:20px;">
	...
	<tr>
		<th style="text-align:left;border:1px solid #e0e0e0;padding:8px 12px;background:#f8f8f8;"><?php esc_html_e( 'Reported products', 'shopforge' ); ?></th>
		<td style="text-align:left;border:1px solid #e0e0e0;padding:8px 12px;">
	...
	<?php endif; ?>
	<tr>
		<th style="text-align:left;border:1px solid #e0e0e0;padding:8px 12px;background:#f8f8f8;"><?php esc_html_e( 'Message', 'shopforge' ); ?></th>
...
<p>
	<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-orders&id=' . $order->get_id() ) ); ?>"
	   style="display:inline-block;background:#96588a;color:#fff;padding:10px 20px;text-decoration:none;border-radius:4px;font-weight:bold;">
		<?php esc_html_e( 'Manage order →', 'shopforge' ); ?>
	</a>
</p>
